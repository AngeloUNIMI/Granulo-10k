...
def _ensure_pointnet2_on_path():
    ...
    candidates = [
        # Self-contained deployment layout used by the Hugging Face Space.
        here,
    ]

    env_project_root = os.environ.get("GRANULO_PROJECT_ROOT")
    if env_project_root:
        env_root = Path(env_project_root).expanduser()
        candidates.extend(
            [
                env_root / "code" / "multimodal_paper",
                env_root / "code" / "multimodal_paper_dinov3",
            ]
        )
