...
def computeMeanStd(dataloader_all, dataset_sizes, batch_sizeP, cuda, stats_path):

    if os.path.isfile(stats_path):
        print("Loaded cached mean/std")
        data = np.load(stats_path)
        mean = data["mean"].item()  # convert scalar array → python float or np.float32
        std  = data["std"].item()
        return mean, std
        ...
        np.savez(stats_path, mean=np.float32(pop_mean), std=np.float32(pop_std0))
